...
interface ImageReader {
  ...
  final class ParcelFileDescriptorImageReader implements ImageReader {
    ...
    @Override
    public ImageHeaderParser.ImageType getImageType() throws IOException {
      // We're wrapping a FileInputStream into a RecyclableBufferedInputStream to optimize I/O
      // performance. It's possible that for certain RAW files the ImageHeaderParserUtils's internal
      // mark() limit is insufficient, and the getType() call fails.
      // TODO(timurrrr): Refactor getType() to just create new RecyclableBufferedInputStream when
      //   reading from a ParcelFileDescriptor; or wrap the FileInputStream in a smarter way.
      InputStream is =
          new RecyclableBufferedInputStream(
              new FileInputStream(dataRewinder.rewindAndGet().getFileDescriptor()), byteArrayPool);
      try {
    ...
    public int getImageOrientation() throws IOException {
      // We're wrapping a FileInputStream into a RecyclableBufferedInputStream to optimize I/O
      // performance. However, for certain RAW files the ImageHeaderParserUtils's internal mark()
      // limit is insufficient, and the getOrientation() call fails.
      // TODO(timurrrr): Refactor getType() to just create new RecyclableBufferedInputStream when
      //   reading from a ParcelFileDescriptor; or wrap the FileInputStream in a smarter way.
      InputStream is =
          new RecyclableBufferedInputStream(
